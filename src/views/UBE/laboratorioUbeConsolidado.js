...
export default function LaboratorioUbeConsolidado() {
    ...
    const listarMunicipio = async () => {
        const data = await start(URL + '/consolidado/listar-municipios')
        setListaMunicipio(data)
    };

    const listar = async () => {
        const data = await start(URL + '/consolidado/listar', { municipio: idMunicipio.campo, corresponde: comunidad.campo })
        setLista(data[0])
    ...
    const buscar = async () => {
        if (comunidad.valido === 'true') {
            const data = await buscarDB(URL + '/consolidado/buscar', { municipio: idMunicipio.campo, corresponde: comunidad.campo, fecha1: inputBuscar.campo, fecha2: inputBuscar2.campo })
            if (data.length > 0) {
                if (data[0].length < 1) {
    ...
    const habilitarEdicion = async (codigo) => {
        if (window.confirm('Habiliar Edicion ?')) {

            const data = await defaultDB(URL + '/consolidado/habilitar-edicion', { codigo })
            listar()
        }
    };
    ...
    const listaExcel = async () => {
        ...
        principal.mergeCells('A8:C11');
        principal.getCell('A8').value = 'COMUNIDAD'
        ...
        for (let d of lista) {
            ...
            principal.mergeCells(`A` + i + `:C` + i);

            principal.getCell(`A` + i).value = d.comunidad
                        ...
                        <CardBody >
                            ...
                            <Table className="tablesorter" responsive>
                                <thead className="text-primary" style={{ border: '3px solid #17a2b8' }}>
                                    <tr>
                                        <th colSpan={1} rowSpan={4} className="tbl-header1" >COMUNIDAD</th>
                                ...
                                <tbody >
                                    {lista.map(e => (
                                        <tr key={e.id}  >
                                            <td style={{ textAlign: 'left', }}>{e.comunidad}  </td>
                                            <td style={{ textAlign: 'left', }}><div className="tbl-edit" onClick={() => {
                                                habilitarEdicion(e.codigo)
                                            }}>habilitar edicion</div>  </td>
