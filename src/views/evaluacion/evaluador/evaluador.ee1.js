...
import { Button, Card, CardHeader, CardBody, Form, Row, Col, Modal, ModalBody, ModalHeader, Table, Alert, } from "reactstrap";
import { InputUsuarioStandar } from "components/input/elementos"; // componente input que incluye algunas de las funcionalidades como, setInput
...
function Evaluacion() {
  ...
  const listarGestion = async () => {
    const listaGEstion = await start(URL + '/evaluador-ee1/listar-anios')
  ...
  const listarMeses = async (gestion) => {
    if (gestion) {
      const listaMeses = await start(URL + '/evaluador-ee1/listar-meses', { gestion })
      setListaMes(listaMeses)
    }
  }

  const listarCasas = async () => {
    if (inputBuscar2.campo) {
      const data = await start(URL + '/evaluador-ee1/listar-casas', { mes: inputBuscar2.campo }, 'Espere, cargando informacion...!')
      if (data[1].length > 0) {
        //console.log(data)
        let totalHoras2 = 0;
        let totalMinutos2 = 0;
    ...
    } else { toast.error('Seleccion el año y el mes que corresponde ') }
  };
  ...
  const insertar = async () => {
    ...
    await editDB(URL + '/evaluador-ee1/guardar', {
      casa: casa.campo,
      inicio: inicio.campo,
      final: final.campo,
      ecin: ecin.campo,
  ...
  const deleted = async (evaluacion) => {
    if (window.confirm('Desea quitar esta evaluacion...?')) {
      await eliminarDB(URL + '/evaluador-ee1/deleted', { evaluacion })
      listarCasas()
    }
  }
  ...
  const listaExcel = async () => {
    ...
    principal.getCell(`A` + (i + 8)).value = 'Evaluador :    ' + localStorage.getItem('nombre')
