...
const InputUsuarioStandar = ({
  ...
  return (
    ...
    <FormGroup>
      ...
      <label id={name} style={{ display: 'none', color: '#FF3D85', fontSize: '9px', fontWeight: 'bold' }}>{msg}</label>
    </FormGroup>
...
const InputUsuarioLogin = ({
  ...
  return (
    <FormGroup>
      ...
      />
      <IconoValidacion valido={estado.valido} icon={estado.valido === 'true' ? faCheckCircle : faTimesCircle} style={{ right: tipo === 'text' ? '10px' : "30px", marginBottom: '0px' }} />
...
const ComponenteCheck_ = ({ name, label, estado, cambiarEstado, f2 = null, f3 = null }) => {
  const onChange = (e) => {
...
const Select1Easy = ({
  ...
  const onChange = (e) => {
    cambiarEstado({ campo: e?.value }); //el valor del campo valido, debe ser una cadena
  ...
  let valor = ''
  for (let e of lista) {
    // console.log(e.label, ' valores iguales fuera de if', estado.campo)

    if (e.value == estado.campo) {
      // console.log(e.label, ' valores iguales ', estado.campo)
...
const ComponenteInputBuscar_ = ({
  ...
  return (
    <Row className="search-container">
      <Col md='5' className="search-input-col">
        <InputBuscador
          type="text"
          value={estado.campo || ""}
          id={"idBuscador"}
      ...
      {window.innerWidth > 768 &&
        <Col md='3' className="search-button-col">
          <Button
            onClick={() => eventoBoton()}
...
const ComponenteSubTitle = ({ etiqueta, contenido, tam = null }) => {
  ...
  return (
    <div > <span style={{ fontWeight: '800', fontSize: tam ? `${tam}px` : '10px' }}>{etiqueta ? etiqueta : null}</span>
      <span style={{ whiteSpace: 'wrap', fontSize: tam ? `${tam}px` : '10px' }}>{contenido}</span>
    </div>
  )
}
...
export {
  ComponenteCheck,
  InputUsuarioStandar, InputUsuarioStandarTable, InputUsuarioLogin, InputTextArea, ComponenteCheck_, ComponenteInputFile,
  Select1Aux,
  ComponenteInputBuscar_,
  Select1, Select1XL, Select1Easy, Select1EasyColors,
  SelectMultiple,
  SelectString,
